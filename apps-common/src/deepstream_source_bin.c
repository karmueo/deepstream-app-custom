...
static gboolean set_camera_v4l2_params(NvDsSourceConfig *config,
                                       NvDsSrcBin       *bin)
{
    gchar device[64];
...
static gboolean create_camera_source_bin(NvDsSourceConfig *config,
                                         NvDsSrcBin       *bin)
{
    ...
    case NV_DS_SOURCE_CAMERA_V4L2:
        ...
        caps1 = gst_caps_new_simple(
            "video/x-raw", "width", G_TYPE_INT, config->source_width, "height",
            G_TYPE_INT, config->source_height, "framerate", GST_TYPE_FRACTION,
    ...
    if (config->video_format)
    {
        caps = gst_caps_new_simple(
            "video/x-raw", "format", G_TYPE_STRING, config->video_format,
    ...
    if (config->type == NV_DS_SOURCE_CAMERA_V4L2)
    {
        ...
        nvvidconv2 =
            gst_element_factory_make(NVDS_ELEM_VIDEO_CONV, "nvvidconv2");
        if (!nvvidconv2)
        {
        ...
        else
        {
            ...
            /* For Jetson, with copy-hw=1 and
               memory-type=nvbuf-mem-surface-array, cudaMemcopy fail is
               observed. This is a WAR till root cause is fixed */
            if (config->type == NV_DS_SOURCE_CAMERA_V4L2)
            {
            ...
            else
            {
                ...
                {
                    g_object_set(G_OBJECT(nvvidconv2), "copy-hw",
                                 config->nvvideoconvert_copy_hw, NULL);
                }
            }
#endif
            gst_bin_add_many(GST_BIN(bin->bin), bin->src_elem, bin->cap_filter1,
...
static void cb_newpad(GstElement *decodebin, GstPad *pad, gpointer data)
{
    ...
    if (!strncmp(name, "video", 5))
    {
        ...
        {
            NvDsSourceConfig *config = (NvDsSourceConfig *)g_object_get_data(
                G_OBJECT(bin->cap_filter), SRC_CONFIG_KEY);
    ...
    else if (g_str_has_prefix(name, "audio/x-raw"))
    {
        ...
        GstPad *sinkpad =
            gst_element_get_static_pad(bin->audio_converter, "sink");
                                  ...
                                  gchar *name, gpointer user_data)
{
    ...
    {
        NVGSTDS_ERR_MSG_V("Failed to get properties for GPU %d",
                          config->gpu_id);
    }
    ...
    if (g_strstr_len(name, -1, "nvv4l2decoder") == name)
    {
        ...
        if (!prop.integrated)
        {
            g_object_set(object, "extract-sei-type5-data",
        ...
        if (config->loop &&
            g_strstr_len(config->uri, -1, "file:/") == config->uri)
        {
            NVGSTDS_ELEM_ADD_PROBE(
                bin->src_buffer_probe, GST_ELEMENT(object), "sink",
...
static gboolean cb_rtspsrc_select_stream(GstElement *rtspsrc, guint num,
                                         GstCaps *caps, gpointer user_data)
{
    ...
    if (!bin->depay)
    {
        ...
        {
            NVGSTDS_ERR_MSG_V("'%s' failed to sync state with parent",
                              elem_name);
            return FALSE;
        }
...
void destroy_smart_record_bin(gpointer data)
{
    ...
    for (i = 0; i < pbin->num_bins; i++)
    {
        ...
        if (src_bin->cleanup_timeout_id)
        {
            g_source_remove(src_bin->cleanup_timeout_id);
            src_bin->cleanup_timeout_id = 0;
        }
...
static gpointer smart_record_callback(NvDsSRRecordingInfo *info,
                                      gpointer             userData)
{
    static GMutex mutex;
    FILE         *logfile = NULL;
    ...
    // 重命名文件为可读的时间格式
    if (info->filename && info->dirpath)
    {
        gchar      old_path[512];
        gchar      new_filename[512];
        gchar      new_path[512];
        gchar      formatted_time[32] = {0};
        gchar      duration_suffix[32];
        time_t     ts = time(NULL);
        struct tm *t = NULL;

        // 构建旧文件的完整路径
        g_snprintf(old_path, sizeof(old_path), "%s/%s", info->dirpath,
                   info->filename);
        ...
        guint64 duration_ms = info->duration < 0 ? 0 : (guint64)info->duration;
        guint64 duration_secs = (duration_ms + 999) / 1000; // 向上取整，避免小于1秒显示为0
        g_snprintf(duration_suffix, sizeof(duration_suffix), "%llus",
                   (unsigned long long)duration_secs);

        g_snprintf(new_filename, sizeof(new_filename),
                   "Smart_Record_%05u_%s_%s%s", info->sessionId,
                   formatted_time, duration_suffix, ext);
        ...
        {
            g_print("Smart Record: The file has been renamed to %s\n",
                    new_filename);
            ...
            if (logfile)
            {
                fprintf(logfile, "%d:%d:%d:%ldms:%s:%s\n", info->sessionId,
                        ...
                        info->dirpath, new_filename);
                fclose(logfile);
            }
        }
        else
        {
            g_print("Smart Record: 重命名失败，保留原文件名 %s (errno: %d)\n",
                    info->filename, errno);
    ...
    else
    {
        ...
        {
            fprintf(logfile, "%d:%d:%d:%ldms:%s:%s\n", info->sessionId,
                    info->width, info->height, info->duration, info->dirpath,
                    info->filename);
            fclose(logfile);
        }
        else
    ...
    /* 定期清理超过保留天数的录像 */
    static gboolean smart_record_cleanup_cb(gpointer data)
    {
        ...
        if (config->smart_rec_retention_days == 0 || !config->dir_path)
            return TRUE;

        time_t now = time(NULL);
        time_t cutoff = now - (time_t)config->smart_rec_retention_days * 24 * 60 * 60;
        const time_t fresh_guard = 5 * 60; /* 5分钟保护，避免删正在写入的文件 */

        DIR *dir = opendir(config->dir_path);
        if (!dir)
            return TRUE;

        const gchar *user_prefix = config->file_prefix ? config->file_prefix : "";
        const gchar *fixed_prefix = "Smart_Record_";
        ...
        while ((ent = readdir(dir)) != NULL)
        {
            ...
            gboolean ext_ok = g_str_has_suffix(name, ".mp4") || g_str_has_suffix(name, ".mkv");
            if (!ext_ok)
                continue;

            if (!(g_str_has_prefix(name, fixed_prefix) ||
                  (user_prefix[0] != '\0' && g_str_has_prefix(name, user_prefix))))
                continue;

            gchar fullpath[1024];
            g_snprintf(fullpath, sizeof(fullpath), "%s/%s", config->dir_path, name);

            struct stat st;
            if (stat(fullpath, &st) != 0 || !S_ISREG(st.st_mode))
                continue;

            if (now - st.st_mtime < fresh_guard)
                continue;

            if (st.st_mtime < cutoff)
            {
                if (remove(fullpath) == 0)
                    g_print("Smart Record Cleanup: deleted %s\n", fullpath);
                else
                    g_print("Smart Record Cleanup: failed delete %s (errno=%d)\n", fullpath, errno);
            }
        }
...
static void check_rtsp_reconnection_attempts(NvDsSrcBin *src_bin)
{
    ...
    for (i = 0; i < src_bin->parent_bin->num_bins; i++)
    {
        ...
        if (src_bin->parent_bin->sub_bins[i].have_eos &&
            (src_bin->parent_bin->sub_bins[i].rtsp_reconnect_interval_sec ==
                 0 ||
        ...
        if (time_since_last_reconnect_sec >= SOURCE_RESET_INTERVAL_SEC)
        {
            if (time_diff_msec_since_last_reset > 3000)
            {
                ...
                else
                {
                    ...
                    {
                        GST_ERROR_OBJECT(src_bin->bin,
                                         "Can't set source bin to NULL");
                    }
            ...
            time_since_last_buf_sec >= src_bin->rtsp_reconnect_interval_sec)
        {
            if (time_diff_msec_since_last_reset > 3000)
            {
                        ...
                        src_bin->rtsp_reconnect_attempts)
                {
                    ...
                    NVGSTDS_WARN_MSG_V("No data from source %d since last %u "
 ...
 */
static GstPadProbeReturn
rtspsrc_monitor_probe_func(GstPad *pad, GstPadProbeInfo *info, gpointer u_data)
{
    NvDsSrcBin *bin = (NvDsSrcBin *)u_data;
    if (info->type & GST_PAD_PROBE_TYPE_BUFFER)
    {
...
static gboolean create_rtsp_src_bin(NvDsSourceConfig *config, NvDsSrcBin *bin)
{
    ...
    if (config->smart_record)
    {
        ...
        if (config->file_prefix)
            params.fileNamePrefix = g_strdup_printf(
                "%s_%d", config->file_prefix, config->camera_id);
        ...
        if (config->smart_rec_retention_days > 0)
        {
            if (bin->cleanup_timeout_id)
            {
                g_source_remove(bin->cleanup_timeout_id);
                bin->cleanup_timeout_id = 0;
            }
            /* 每小时清理一次 */
            bin->cleanup_timeout_id =
                g_timeout_add_seconds(3600, smart_record_cleanup_cb, bin);
            g_print("Smart Record Cleanup: scheduled hourly cleanup for %s, retention=%u days\n",
                    config->dir_path ? config->dir_path : "(null)",
    ...
    if (config->dewarper_config.enable)
    {
        ...
        gst_bin_add_many(GST_BIN(bin->bin), bin->src_elem,
                         bin->tee_rtsp_pre_decode, bin->dec_que, bin->decodebin,
                         bin->cap_filter, bin->tee_rtsp_post_decode,
                         bin->dewarper_bin.bin, NULL);
    }
    else
    {
        g_snprintf(elem_name, sizeof(elem_name), "nvvidconv_elem%d",
                   bin->bin_id);
        bin->nvvidconv =
        ...
        bin->cap_filter1 = gst_element_factory_make(NVDS_ELEM_CAPS_FILTER,
...
static gboolean create_audiodecode_src_bin(NvDsSourceConfig *config,
                                           NvDsSrcBin       *bin)
{
    gboolean    ret = FALSE;
    ...
    {
        bin->src_elem =
            gst_element_factory_make(NVDS_ELEM_SRC_ALSA, "src_elem");
        if (!bin->src_elem)
        {
            NVGSTDS_ERR_MSG_V("Could not create element 'src_elem'");
            goto done;
    ...
    bin->cap_filter = gst_element_factory_make(NVDS_ELEM_CAPS_FILTER,
    ...
    if (config->type == NV_DS_SOURCE_AUDIO_WAV)
    {
        ...
        gst_element_link_many(bin->src_elem, bin->decodebin,
                              bin->audio_converter, bin->audio_resample,
                              bin->cap_filter, NULL);
    }
    else if (config->type == NV_DS_SOURCE_ALSA_SRC)
...
static gboolean create_uridecode_src_bin_audio(NvDsSourceConfig *config,
                                               NvDsSrcBin       *bin)
{
    gboolean    ret = FALSE;
    guint const MAX_CAPS_LEN = 256;
    ...
    bin->audio_resample = gst_element_factory_make(NVDS_ELEM_AUDIO_RESAMPLER,
                                                   "audioresampler_elem");
    if (!bin->audio_resample)
    {
...
static gboolean create_uridecode_src_bin(NvDsSourceConfig *config,
                                         NvDsSrcBin       *bin)
{
    gboolean         ret = FALSE;
    GstCaps         *caps = NULL;
...
gboolean create_multi_source_bin(guint num_sub_bins, NvDsSourceConfig *configs,
                                 NvDsSrcParentBin *bin)
{
    ...
    for (i = 0; i < num_sub_bins; i++)
    {
        ...
        {
            NVGSTDS_ERR_MSG_V(
                "source %d cannot be linked to mux's sink pad %p\n", i,
                bin->streammux);
            goto done;
        }
    ...
    {
        NVGSTDS_ELEM_ADD_PROBE(bin->nvstreammux_eosmonitor_probe,
                               bin->streammux, "src",
...
static void set_properties_nvuribin(GstElement             *element_,
                                    NvDsSourceConfig const *config)
{
    GstElementFactory *factory =
        GST_ELEMENT_GET_CLASS(element_)->elementfactory;
    ...
    if (config->drop_frame_interval)
        g_object_set(element_, "drop-frame-interval",
                     config->drop_frame_interval, NULL);
    if (config->extract_sei_type5_data)
        g_object_set(element_, "extract-sei-type5-data",
                     config->extract_sei_type5_data, NULL);
    if (config->select_rtp_protocol)
        g_object_set(element_, "select-rtp-protocol",
                     config->select_rtp_protocol, NULL);
    if (config->loop)
    ...
    if (config->smart_rec_container)
        g_object_set(element_, "smart-rec-container",
                     config->smart_rec_container, NULL);
    ...
    if (config->udp_buffer_size)
        g_object_set(element_, "udp-buffer-size", config->udp_buffer_size,
                     NULL);
}
                                     ...
                                     NvDsSrcParentBin *bin)
{
    ...
    bin->nvmultiurisrcbin = bin->streammux = gst_element_factory_make(
        NVDS_ELEM_NVMULTIURISRCBIN, "src_nvmultiurisrcbin");
    if (!bin->streammux)
    {
