...
parse_sink_yaml(NvDsSinkSubBinConfig *config, std::string group_str, gchar *cfg_file_path)
{
         ...
         itr != sink_node.end(); ++itr)
    {
        ...
        {
            config->cuav_control_config.pt_focal_step = itr->second.as<gdouble>();
        }
        ...
        {
            parse_cuav_corner_zoom_cycle_yaml(&config->cuav_control_config, itr->second);
        }
        else if (paramKey == "home-servo")
        {
            parse_cuav_home_servo_yaml(&config->cuav_control_config, itr->second);
        }
        ...
        {
            if (itr->second.IsMap())
                parse_cuav_startup_pt_focal_yaml(&config->cuav_control_config, itr->second);
            else
                config->cuav_control_config.startup_pt_focal = itr->second.as<guint>();
        }
