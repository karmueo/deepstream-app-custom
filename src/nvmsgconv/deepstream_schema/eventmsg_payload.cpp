...
generate_object_object(void *privData, NvDsEventMsgMeta *meta)
{
    ...
    json_object_set_double_member(objectObj, "confidence", meta->confidence);
                ...
                if (dsObj)
                {
                    ...
                    if (meta->pose.num_joints)
                    {
                        if (meta->pose.pose_type == 0)
                        {
                            ...
                            {
                                std::string s = _joint_maps[meta->pose.pose_type][joint_index] + "," + to_string(meta->pose.joints[joint_index].x) + "," + to_string(meta->pose.joints[joint_index].y) + "," + to_string(meta->pose.joints[joint_index].confidence);
                                ss << s << "|";
                            }
                        }
                        else if (meta->pose.pose_type == 1)
                        {
                            ...
                            for (joint_index = 0; joint_index < meta->pose.num_joints; joint_index++)
                            {
                                std::string s = _joint_maps[meta->pose.pose_type][joint_index] + "," + to_string(meta->pose.joints[joint_index].x) + "," + to_string(meta->pose.joints[joint_index].y) + "," + to_string(meta->pose.joints[joint_index].z) + "," + to_string(meta->pose.joints[joint_index].confidence);
                                ss << s << "|";
                            }
                        }
