...
nvds_msg2p_parse_sensor(void *privData, GKeyFile *key_file, gchar *group)
{
    ...
    for (key = keys; *key; key++)
    {
        ...
        {
            gsize length;
            gdouble *location = g_key_file_get_double_list(key_file, group, CONFIG_KEY_LOCATION, &length, &error);
            if (length != 3)
            {
        ...
        {
            gsize length;
            gdouble *coordinate = g_key_file_get_double_list(key_file, group, CONFIG_KEY_COORDINATE, &length, &error);
            if (length != 3)
            {
...
static bool
nvds_msg2p_parse_datamap_yaml(void *privData, gchar *cfg_file_path, std::string group_str)
{
    ...
    if (configyml[group_str]["fused_detection_key"])
        privObj->datamapCfg.obj_key_fusion = configyml[group_str]["fused_detection_key"].as<string>();
    ...
    if (configyml[group_str]["lidar_element_size"])
        privObj->datamapCfg.lidar_element_size = configyml[group_str]["lidar_element_size"].as<guint32>();
    if (configyml[group_str]["lidar_element_max_points"])
        privObj->datamapCfg.lidar_element_max_points = configyml[group_str]["lidar_element_max_points"].as<guint32>();
    return true;
}
