/**
 * @file deepstream_app_config_parser.c
 * @brief DeepStream 应用程序 INI 格式配置文件解析器。
 *
 * 从 GKeyFile 格式的配置文件中解析应用程序、视频源、
    ...
    for (i = 0; i < config->total_num_sources; i++)
    {
        ...
        if (config->uri_list)
        {
            ...
            else
            {
                ...
                {
                    NVGSTDS_ERR_MSG_V("Incorrect URI for camera source %s. FORMAT: <usb/csi>:<dev_node/sensor_id>",
                                      uri);
                    return FALSE;
                }
    ...
    for (group = groups; *group; group++)
    {
                     ...
                     sizeof(CONFIG_GROUP_SOURCE) - 1))
        {
            ...
            if (config->source_list_enabled)
            {
                ...
                {
                    NVGSTDS_ERR_MSG_V("Invalid source group index %d, index cannot exceed %d", index,
                                      config->total_num_sources);
                    ret = FALSE;
                    goto done;
                }
            ...
            if (config->source_list_enabled && config->multi_source_config[source_id].type ==
                     ...
                     sizeof(CONFIG_GROUP_SECONDARY_GIE) - 1))
        {
             ...
             * it will override the value set using global_gpu_id in parse_gie function */
            parse_err =
                !parse_gie(&config->secondary_gie_sub_bin_config[config->num_secondary_gie_sub_bins],
                           cfg_file, *group, cfg_file_path);
                     ...
                     sizeof(CONFIG_GROUP_SECONDARY_PREPROCESS) - 1))
        {
            ...
            parse_err =
                !parse_preprocess(&config->secondary_preprocess_sub_bin_config
                                       [config->num_secondary_preprocess_sub_bins],
                                  cfg_file, *group,
                                  cfg_file_path);

            if (config->secondary_preprocess_sub_bin_config[config->num_secondary_preprocess_sub_bins].enable)
            {
                config->num_secondary_preprocess_sub_bins++;
            }
        }
        ...
        if (!strncmp(*group, CONFIG_GROUP_SINK, sizeof(CONFIG_GROUP_SINK) - 1))
        {
            ...
            parse_err =
                !parse_sink(&config->sink_bin_sub_bin_config[config->num_sink_sub_bins], cfg_file, *group, cfg_file_path);
                     ...
                     sizeof(CONFIG_GROUP_MSG_CONSUMER) - 1))
        {
            ...
            parse_err =
                !parse_msgconsumer(&config->message_consumer_config[config->num_message_consumers], cfg_file, *group, cfg_file_path);
        ...
        if (!strncmp(*group, CONFIG_GROUP_DEWARPER, strlen(*group) - 1))
        {
            guint source_id = 0;
            {
                ...
                if (source_id_start_ptr == source_id_end_ptr || *source_id_end_ptr != '\0')
                {
                    NVGSTDS_ERR_MSG_V("dewarper group \"[%s]\" is not in the form \"[dewarper<%%d>]\"", *group);
                    ret = FALSE;
                    goto done;
                }
            }
