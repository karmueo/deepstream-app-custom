/**
 * @file deepstream_app.c
 * @brief DeepStream GStreamer 分析流水线的构建与生命周期管理。
 *
 * 负责根据配置创建完整的 GStreamer 流水线，包括视频源、推理引擎、
 * 跟踪器、OSD、Sink 等组件的创建与链接，以及流水线的暂停、恢复、
 * 销毁等生命周期操作。
 */
    ...
    *sink_elem = *src_elem = NULL;
    ...
    if (config->udpjsonmeta_config.enable)
    {
        ...
        if (config->udpjsonmeta_config.enable_cuav_parser)
        {
            ...
            register_cuav_udpjson_callbacks(udpjsonmeta,
